...
2. Tenés dos caminos: botón visible **Subir foto** (abajo a la izquierda) o botón **Upload Image** en la GUI.
3. También podés arrastrar la imagen al recuadro punteado o pegarla con `Ctrl/Cmd + V`.
4. Elegí un archivo (`image/*`, por ejemplo PNG/JPG).
5. La textura se aplica automáticamente al shader; si no cargás ninguna imagen se usa un **placeholder degradado** para evitar fondo negro.
...
## Dónde pegar o soltar la fotografía

- Hay un panel fijo llamado **Imagen para editar** en la esquina inferior izquierda.
- Incluye: botón **Subir foto**, zona de **drag & drop** y soporte de **pegar desde portapapeles**.
- El texto de estado te avisa si la imagen cargó bien o si hubo un error.
